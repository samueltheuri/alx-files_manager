...
/**
 * GGenerates the thumbnail of an image with a given width size.
...
fileQueue.process(async (job, done) => {
	...
	if (!userId) {
		throw new Error('Missimg fileId');
	}
	if (!userId) {
...
userQueue.process(async (job, done) => {
	...
	try {
		cinst mailSubject = 'Welcome to ALX-Files_Manager by mickybe';
		const mailContent = [
			'<div>',
			'<h3>Hello {{user.name}},</h3>',
			'Welcome to <a href="https://github.com/samueltheuri/alx-files_manager">',
			'ALX-Files_Manager</a>, ',
			'a simple file management API built with Node.js by ',
			'<a href="https://github.com/samueltheuri">Samuel Muriuki</a>. ',
